from download_yt.config import USER
...
def get_ffmpeg_cmd():
    if getattr(sys, 'frozen', False):
        return str(Path(sys.executable).parent / "ffmpeg.exe")
    else:
        return str(Path(__file__).parent / "ffmpeg.exe") if (Path(__file__).parent / "ffmpeg.exe").exists() else "ffmpeg"
